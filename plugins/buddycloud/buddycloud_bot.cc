...
void BuddycloudBot::GetContactChannel(Contact::Ref contact) {
  ...
  if (channel) {
    ...
    channel->onReady.connect([&] (ChannelController* controller) {
...
BcContact::Ref BuddycloudBot::GetContact(const Swift::JID &jid) {
  // TODO: change to new standard for
...
void ChannelController::GetChannelContent() {
...
void ChannelController::Send(Post1::Ref post) {
  ...
  rq->onResponse.connect([&] (PubsubPublishRequest::ref publish, Swift::ErrorPayload::ref error) {
    if (error) {
...
void ChannelController::Delete(Post1::Ref post) {
  ...
  retract->onResponse.connect([&] (PubsubRetractRequest::ref retract, ErrorPayload::ref error) {
    if (error) {
      LOG(ERROR) << error->getText();
      return;
    }
